<script>
  ...
  function getInitialText(index) {
    return `${unassignedIssuesArray[index] !== null && totalIssuesArray[index] !== null 
      ? `${unassignedIssuesArray[index]}/${totalIssuesArray[index]}` 
      : 'Loading...'}`;
  }

  function getHoverText(index) {
    return `Available issues: ${unassignedIssuesArray[index]}<br>Total issues: ${totalIssuesArray[index]}<br>Maintainer: xyz`;
  }

  onMount(function () {
    repoNames.forEach(function (repoName, i) {
      getIssueCount(repoName, i);
    });
  });
</script>
  ...
  <div class="mx-10 my-5 bg-black py-2 rounded-lg">
    <div class="grid grid-cols-1 lg:grid-cols-3">
      {#each repos as repo, i}
        <a
          href={repo}
          class="flex justify-between text-center my-3 mx-12 text-[#183717] pr-4 text-[20px] bg-[#D8FFD8] py-3"
          target="_blank"
          on:mouseenter={() => hovering = i}
          on:mouseleave={() => hovering = null}
        >
          <button class="w-full h-full truncate">{repoNames[i]}</button>
          <p class="text-[12px]">
